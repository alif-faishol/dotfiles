...
# dotfiles boostrap
#
# source: https://github.com/ActionScripted/dotfiles/blob/master/bootstrap.sh
#
...
backup_dotfile() {
  if [[ -e "$HOMEDIR/$1"  ]]; then
          ...
          dotfile_count=$(find $BACKUPDIR/$1* -maxdepth 0 2> /dev/null | wc -l |
            sed 's/ //g')
...
if [[ $SUDO_USER  ]]; then
  echo "Running scripts you find on the internet as root is dangerous. Try again without
  'sudo'."
  exit 1
fi
